...
void setup()
{
    ...
    Timer1.disablePwm(10);
        ...
        onTime = map(dim, 0, 100, 0, period/2);	// re scale the value from hex to uSec 
        offTime = period/2 - onTime;
...
void loop() {			// Non time sensitive tasks - read the serial port
        ...
        else if (state==2) //vinc del nowisthetime
        {
                  Timer1.detachInterrupt();
                   Timer1.attachInterrupt(nowIsTheTime2,onTime);
                   state=10;
        }
}
