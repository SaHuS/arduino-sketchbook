...
int squirtel=  3276700000;                      //attached or detached while in the main loop
...
void setup()
{
    ...
    Timer1.disablePwm(10);
        ...
        onTime = map(dim, 0, 1000, 0, period);	// re scale the value from hex to uSec 
        offTime = onTime+1000;
        Serial.print("period=");	// feeling talkative?	
        Serial.println(period);
        Serial.print("onTime=");	// feeling talkative?	
        Serial.println(onTime);
        Serial.print("offTime=");	// feeling talkative?	        
...
//    if (dim>=95)			//if off time is very small
//    {
...
//        #endif
//    }
//    else if (dim<=5) {		//if offTime is large
...
void loop() {			// Non time sensitive tasks - read the serial port
        ...
        if(state==B00000011)	//despres del zero crosing
        {

        Timer1.attachInterrupt(nowIsOnTime,onTime);
            state=B00000001;	//when it is the time for nowIsTheTime the state will align with unity
        }
        else if(state==B00000010)	//despres d'apagar el triac vinc del nowisthetime2
        {
 ...
 //          Timer1.detachInterrupt();
        ...
        else if (state==200) //vinc del nowisthetime
        {
        Timer1.detachInterrupt();
                                     Timer1.restart();
                   Timer1.attachInterrupt(nowIsOffTime,4170);
